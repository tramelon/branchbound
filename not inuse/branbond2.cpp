...
// add out mas problem, form whole algorithm, 10/1/2014
...
{  
  ...
  BB_LB = -HUGE_VAL;   
  BB_gap = 1;
  BB_gap0 = 0.001; // tolarable gap
  node_gap0=0.1*BB_gap0; // node tolarable gap
}
...
float branbond2(int tempu[][T1+1], int tempv[][T1+1], int tempw[][T1+1]) // main funciton of Branch and Bound
	{				
	//main body
	IloEnv env;
try {
...
//	parinit();
	...
	while(1)
	{
// calcate roote node, if optimal then break;
