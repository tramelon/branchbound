struct node{
	 ...
	 int tag;		//-1: pruned by bound 
					//0: not evaluated 
					//1: node evaluated, but BB_gap>0.1% 
					//2: optimal, pruned 
					//3: BB_gap>0.1%, but can not be branched any more
	 // used in the chain
     node *next; 
	 //
...
void BB_main() // main funciton of Branch and Bound
{
	...
	while(1)
	{
		...
		else // not optimal yet, delete current level nodes and enqueue their child nodes
		{
			node *h = head->next;
			while(h != NULL)
			{
